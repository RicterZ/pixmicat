...
class PIOsqlite3 implements IPIO {
		...
		switch($action) {
			case 'optimize':
				if($doit){
					...
					if($this->con->query('VACUUM '.$this->tablename)!==false) return true;
					else return false;
	...
	public function dbImport($data){
		...
		for($i = 0; $i < $data_count; $i++){
			...
			$PDOStmt->queryute() or $this->_error_handler('Insert a new post failed', __LINE__);
		}
		$this->dbCommit(); // 送交
		return true;
	}
	...
	public function removePosts($posts){
		...
		if(!$this->con->query('DELETE FROM '.$this->tablename.' WHERE no IN ('.$pno.') OR resto IN('.$pno.')')) $this->_error_handler('Delete old posts and replies failed', __LINE__);
		return $files;
	}
	...
	public function addPost($no, $resto, $md5chksum, $category, $tim, $ext, $imgw, $imgh, $imgsize, $tw, $th, $pwd, $now, $name, $email, $sub, $com, $host, $age=false, $status=''){
		...
		if($resto){ // 新增回應
			...
			if($age){ // 推文
				...
				$result->queryute(array(':now' => $updatetime, ':resto' => $resto)) or $this->_error_handler('Push the post failed', __LINE__);
			}
	...
	public function searchPost($keyword, $field, $method){
        ...
        if ($this->con->exec($SearchQuery) === false) {
        	die('SQL Injection!');
        }
	...
	public function searchCategory($category){
		...
		$result->queryute(array(':category' => '%,'.strtolower($category).',%'));
	...
	public function updatePost($no, $newValues){
		...
		foreach($chk as $c){
			if(isset($newValues[$c])){
				if(!$this->con->query('UPDATE '.$this->tablename." SET $c = ".$this->con->quote($newValues[$c]).' WHERE no = '.$no))
